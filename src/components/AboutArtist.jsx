...
const AboutArtist = ({ artist, songs, setCurrentSong, isPlaying, audioRef, setSongs, listQueue, setListQueue, typeOfButton }) => {
    const artistSongs = songs.filter(song => song.artist === artist.artist);

    const playAllSongs = () => {
        setListQueue(artistSongs);
    ...
    return (
        <div className="about-artist" style={{ 
            padding: '20px', 
            color: 'white', 
                        ...
                        {artistSongs.map((song, index) => (
                            <tr key={song.id} className='bgHover' style={{ cursor: 'pointer' }} onClick={() => setCurrentSong(song)}>
                                ...
                                <td style={{ textAlign: 'right', padding: '10px' }}>
                                    <button className='iconAddToQueueFromAboutArtist' onClick={addToQueue(song)}>
